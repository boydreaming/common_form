...
//===========
 外部程序的变量说明：1.(version 3 的表必须含有一个primary_key的字段)
...
//=================================
	function showmainbutton() {
		global $common_style_table,$common_style_addbutton;
  	...
  	$result=$this->executesql($common_sqlstr);
 		...
 		echo "\n<TABLE $common_style_table align=center>";
  	...
  	if (strpos($this->type,"list|")>0) {
  		echo "<BUTTON $common_style_button onclick=window.location.href='$parent?common_mod=list&common_curpage=1&common_condition=".$this->listcondition.$this->oldurl."'>浏 览</BUTTON>";
		}
	...
	//借$common_first为过滤变量，控制《增加》按钮的有效性
  	...
  	if (strpos($this->type,"add|")>0)
  		echo "<BUTTON $common_style_addbutton $common_first>".$this->str_add."</BUTTON>";
  	if (strpos($this->type,"filter|")>0)
  		echo "<BUTTON $common_style_button onclick=window.location.href='$parent?common_mod=filter&common_condition=$tmp".$this->oldurl."'>筛 选</BUTTON>";
  	if (strpos($this->type,"search|")>0){
  		...
  		echo "<INPUT type='text' name=common_searchkey value=\"".$this->searchkey."\" title=$temp><INPUT TYPE=SUBMIT value='搜 索' $common_style_button >";
  	}
  	if ($this->mainindex!="")
  		echo "<INPUT TYPE='BUTTON' name='exit' value='退 出' $common_style_button onclick=window.location.href='$mainindex'>\n";
...
function extinput($value,$type,$fact,$factarray,$rel_array,$appendstyle,$size) {
  if ($size>=40)
	...
	if (is_array($rel_array[$value])) { //多维数组
		...
		$last.="<INPUT name=$value VALUE='$fact' type=text  size=$size $appendstyle><button type=button $common_style_button onClick='getpopval($value,$value"."_list".")'>...</button>";
	}else{ //二维数组及一维
...
if (!isset($common_style_table))
	$common_style_table=" width=100% border=1 ";
...
}elseif (!empty($_REQUEST['common_condition'])) { 
	//2015/7/20 既判断直接传变量common_condition没必须再次判断$_request，且两次stripslashes会导致'aaa'缺失一个单引号
...
//=================================
  if ($common_mod=="edit") {
    ...
    for ($i=0;$i< $common->cols;$i++) {
     ...
     echo "<TR><TH colspan=2><INPUT TYPE='SUBMIT' name='add' value='确认'><TH></TR></TABLE></FORM>\n";
  }
...
//=================================
  if ($common_mod=="add") {
    ...
    echo "<TR><TH><INPUT TYPE='SUBMIT' name='add' value='确认'></TH>";
    echo "<TH><button $common_style_button onclick='javascript:history.go(-1)'>返回</button></TH></TR></TABLE></FORM>\n";
  }
...
//=================================
  if (($common_mod=='list')||($common_mod=="postfilter")||($common_mod=="search")) {
  	/*postfilter另一种用法，比如需要显示“我的申请”，可先对相关字段赋值，无关字段赋_common_all,且所有字段需要在
  	common_fields已定义，且common_ap_未定义隐藏
  	举例如下：
  	$common_fields="id,truename,bureau,requesttime,overtime,outhere,reason";
  	$common_ap_id="hidden";
		if ($common_mod=="listowner") {
			$common_condition="";
			$_POST["truename"]=$uservalue["truename"];
			$_POST["bureau"]=$_POST["requesttime"]=$_POST["overtime"]=$_POST["starttime"]=$_POST["outhere"]=$_POST["reason"]=$_POST["loghashid"]=$_POST["state"]="_common_all";
			$_GET["common_mod"]="postfilter";
		}
    ...
    $result=$common->executesql($common_sqlstr);
    echo "\n<div align=center ><TABLE $common_style_table>\n";
    ...
    if (strpos($common_type,"del|")>0)
    	echo "<TH ><input type=button style='width:52px' onclick=\"{ if (confirm('确定删除记录吗?')) {document.common_frm.submit();return true;}return false;}\" value=\"删除\"></TH>";
    else
    	echo "<TH>&nbsp;</TH>";
     //字段显示并翻译为中文
    for ($i=0;$i<$common->cols;$i++) {
    	...
    	if ((!preg_match("/_data_/i",$value))&&($appendstyle<>"hidden")) {
      	...
      	echo "<TH align='center'><B>$value</B></TH>";
    	}
    }
    echo "</TR>";
    //显示记录内容
        ...
        if ($appendstyle<>"hidden") {
      		...
      		if (isset($$tmp)) {
      			if (strpos($tmp1,"#common")>0) {
      				...
      				$value="<div align=center><button $common_style_button onclick=window.location='".$$tmp."&common_mod=list&common_id=$common_id'>$tmp1</button></div>";
      			}
      		}
